...
static void
echo_client_on_hsk_done (lsquic_conn_t *conn, enum lsquic_hsk_status status)
{
    clock_t difference = clock() - handshake_start;
    ...
    {
        printf("Handshake failed because of session resumption, will retry without it\n");
    }
    else
        assert(0);
...
const struct lsquic_stream_if client_echo_stream_if = {
    ...
    .on_hsk_done            = echo_client_on_hsk_done,
};
