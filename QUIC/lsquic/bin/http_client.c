...
lsquic_time_t connection_created;
...
static void
create_connections_ver (struct http_client_ctx *client_ctx, enum lsquic_version ver)
{
    size_t len;
    FILE *file;
    unsigned char sess_resume[0x2000];
    ...
    while (client_ctx->hcc_n_open_conns < client_ctx->hcc_concurrency &&
           client_ctx->hcc_total_n_reqs > 0)
        if (0 != prog_connect_ver(client_ctx->prog, len ? sess_resume : NULL, len, ver))
        {
            LSQ_ERROR("connection failed");
            exit(EXIT_FAILURE);
        }
...
http_client_on_new_conn (void *stream_if_ctx, lsquic_conn_t *conn)
{
    ...
    connection_created = conn_h->ch_created;
    return conn_h;
}
...
http_client_on_read (lsquic_stream_t *stream, lsquic_stream_ctx_t *st_h)
{
    ...
    do
    {
        if (g_header_bypass && !(st_h->sh_flags & PROCESSED_HEADERS))
        {
            ...
            update_sample_stats(&s_stat_ttfb, st_h->sh_ttfb - connection_created);
...
main (int argc, char **argv)
{
    ...
    enum lsquic_version ver = N_LSQVER;
                                                                            ...
                                                                            )))
    {
        ...
        case 'v':
            if(!strcmp(optarg, "Q043")){
               printf("Version selected: Q043\n\n");
               strcpy(version,optarg);
            	ver = LSQVER_043;
            }
            if(!strcmp(optarg, "Q046")){
               printf("Version selected: Q046\n\n");
		strcpy(version,optarg);
            	ver = LSQVER_046;
            }
            if(!strcmp(optarg, "Q050")){
               printf("Version selected: Q050\n\n");
               strcpy(version,optarg);
            	ver = LSQVER_050;
            }
            if(!strcmp(optarg, "ID27")){
               printf("Version selected: ID27\n\n");
               strcpy(version,optarg);
            	ver = LSQVER_ID27;
            }
            if(!strcmp(optarg, "ID29")){
               printf("Version selected: ID29\n\n");
               strcpy(version,optarg);
            	ver = LSQVER_ID29;
            }
            if(!strcmp(optarg, "I001")){
               printf("Version selected: I001\n\n");
               strcpy(version,optarg);
            	ver = LSQVER_I001;
            }
            break;
    ...
    //prog.prog_settings.es_versions = ver;
    ...
    if (client_ctx.qif_file)
    {
        if (0 != prog_connect_ver(&prog, NULL, 0, ver))
        {
            LSQ_ERROR("connection failed");
            exit(EXIT_FAILURE);
        }
        ...
        create_connections_ver(&client_ctx, ver);
    ...
    if (stats_fh)
    {
        strcpy(filename1, "output/");
        strcat(filename1, prog.prog_hostname);
    	strcat(filename1, "_");
    	strncat(filename1, version, 4);
    	strcat(filename1, "_ct.txt");
	    	
	strcpy(filename2, "output/");    	
    	strcat(filename2, prog.prog_hostname);
    	strcat(filename2, "_");
    	strncat(filename2, version, 4);
        ...
        fprintf(f1,"%.2Lf\n",(long double) s_stat_to_conn.min/1000);
        fprintf(f2,"%.2Lf\n",(long double) s_stat_ttfb.min/1000);        
    }
